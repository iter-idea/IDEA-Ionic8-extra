...
export class IDEAPDFTemplateComponent {
   ...
   */
  @Input() public blueprint: IdeaX.PDFTemplateBlueprint;
  /**
   * The PDF template to manage, as list of sections.
   */
  @Input() public template: Array<IdeaX.PDFTemplateSection>;
  /**
   ...
   */
  public _template: Array<IdeaX.PDFTemplateSection>;
  /**
   ...
   */
  public ST = IdeaX.PDFTemplateSectionTypes;
  /**
  ...
  public ngOnInit() {
    // work on a copy
    this._template = (this.template || []).map(section => new IdeaX.PDFTemplateSection(section, this.languages));
   ...
   */
  public getCurrentTemplate(): Array<IdeaX.PDFTemplateSection> {
   ...
   */
  public openInnerSection(section: IdeaX.PDFTemplateSection) {
  ...
  public addSection() {
    ...
    buttons.push({
      ...
      icon: 'apps',
      handler: () => this.addSectionHelper(new IdeaX.PDFTemplateSection({ type: this.ST.ROW }, this.languages))
    });
    buttons.push({
      text: this.t._('IDEA.PDF_TEMPLATE.HEADER'),
      icon: 'bookmark',
      handler: () => this.addSectionHelper(new IdeaX.PDFTemplateSection({ type: this.ST.HEADER }, this.languages))
    });
    buttons.push({
      text: this.t._('IDEA.PDF_TEMPLATE.BLANK_ROW'),
      icon: 'code',
      handler: () => this.addSectionHelper(new IdeaX.PDFTemplateSection({ type: this.ST.BLANK_ROW }, this.languages))
    });
    buttons.push({
      text: this.t._('IDEA.PDF_TEMPLATE.PAGE_BREAK'),
      icon: 'code-slash',
      handler: () => this.addSectionHelper(new IdeaX.PDFTemplateSection({ type: this.ST.PAGE_BREAK }, this.languages))
    });
   ...
   */
  private addSectionHelper(section: IdeaX.PDFTemplateSection) {
   ...
   */
  public deleteSection(section: IdeaX.PDFTemplateSection) {
    this.alertCtrl
      .create({
   ...
   */
  public getInnerSectionIcon(section: IdeaX.PDFTemplateSection): string {
   ...
   */
  public label(label: IdeaX.Label): string {
   ...
   */
  public editHeaderLabel(label: IdeaX.Label) {
    // skip if we are handling other operations
    if (this.isViewLocked()) return;
    // open the labeler
    this.modalCtrl
   ...
   */
  public addField(section: IdeaX.PDFTemplateSection, indexCol: number) {
   ...
   */
  private addSimpleField(section: IdeaX.PDFTemplateSection, colIndex: number) {
    ...
    this.pickVariableFromList(
      variable => (section.columns[colIndex] = new IdeaX.PDFTemplateSimpleField(variable, this.languages))
    );
  }
  /**
   * Add a complex field to the selected section of type ROW.
   */
  private addComplexField(section: IdeaX.PDFTemplateSection, colIndex: number) {
    // init the content of a new complex field
    const content = new IdeaX.Label(null, this.languages);
   ...
   */
  private pickVariableFromList(callback: (variable: IdeaX.LabelVariable) => void) {
    // prepare the list of suggestions for the picker
    const suggestions = this.getCurrentLayer().blueprint.variables.map(
      v => new IdeaX.Suggestion({ value: v.code, name: this.label(v.label) })
    );
    this.modalCtrl
      .create({
   ...
   */
  public displayField(section: IdeaX.PDFTemplateSection, colIndex: number, forceLabel?: boolean): string {
    // skip in case the column doesn't contain a field
    if (!section.doesColumnContainAField(colIndex)) return;
    const field = section.columns[colIndex] as IdeaX.PDFTemplateComplexField | IdeaX.PDFTemplateSimpleField;
    // recognize whether we are dealing with a complex or a simple field
    if (field.isComplex()) return this.label((field as IdeaX.PDFTemplateComplexField).content);
    else
      return forceLabel
        ? this.label((field as IdeaX.PDFTemplateSimpleField).label)
        : (field as IdeaX.PDFTemplateSimpleField).code.slice(1);
  }

  /**
   * Manage a field (ask what to do).
   */
  public manageField(section: IdeaX.PDFTemplateSection, colIndex: number, event?: any) {
    // skip if we are handling other operations
    if (this.isViewLocked()) return;
    // skip in case the column doesn't contain a field
    if (!section.doesColumnContainAField(colIndex)) return;
   ...
   */
  private openField(section: IdeaX.PDFTemplateSection, colIndex: number) {
    // identify the field
    const field = section.columns[colIndex] as IdeaX.PDFTemplateComplexField | IdeaX.PDFTemplateSimpleField;
      ...
      .create({
        ...
        componentProps: {
          label: field.isComplex()
            ? (field as IdeaX.PDFTemplateComplexField).content
            : (field as IdeaX.PDFTemplateSimpleField).label,
   ...
   */
  private moveField(section: IdeaX.PDFTemplateSection, colIndex: number) {
   ...
   */
  public completeMove(newSection: IdeaX.PDFTemplateSection, newColIndex: number) {
   ...
   */
  private resizeField(section: IdeaX.PDFTemplateSection, colIndex: number, event?: any) {
   ...
   */
  private deleteField(section: IdeaX.PDFTemplateSection, colIndex: number) {
    this.alertCtrl
      .create({
  ...
  public save() {
    ...
    this._template.forEach(section => this.template.push(new IdeaX.PDFTemplateSection(section, this.languages)));
    this.close();
  }
  /**
...
export interface PDFTemplateLayer {
   ...
   */
  blueprint: IdeaX.PDFTemplateBlueprint | IdeaX.PDFTemplateSectionBlueprint;
  /**
...
export interface MoveModeData {
   ...
   */
  section: IdeaX.PDFTemplateSection;
  /**
