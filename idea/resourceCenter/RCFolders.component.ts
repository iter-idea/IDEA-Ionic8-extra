...
export class IDEARCFoldersComponent {
   ...
   */
  public filteredFolders: Array<IdeaX.RCFolder>;
  /**
  ...
  public loadFolders(getFromNetwork?: boolean) {
    this.API.getResource(`teams/${this.teamId}/folders`, {
      ...
      .then((folders: Array<IdeaX.RCFolder>) => {
        this.folders = folders.map(f => new IdeaX.RCFolder(f));
   ...
   */
  public openFolder(folder: IdeaX.RCFolder) {
    this.modalCtrl
   ...
   * Rename a folder.
   */
  public renameFolder(folder: IdeaX.RCFolder, event?: any) {
   ...
   * Delete a folder.
   */
  public deleteFolder(folder: IdeaX.RCFolder, event?: any) {
    if (event) event.stopPropagation();
    if (!this.admin) return;
    this.alertCtrl
      .create({
