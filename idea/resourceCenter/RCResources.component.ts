...
export class IDEARCResourcesComponent {
   ...
   */
  @Input() public folder: IdeaX.RCFolder;
  /**
   ...
   */
  public filteredResources: Array<IdeaX.RCResource>;
  /**
  ...
  public loadResources(getFromNetwork?: boolean) {
    this.API.getResource(`teams/${this.teamId}/folders/${this.folder.folderId}/resources`, {
      ...
      .then((resources: Array<IdeaX.RCResource>) => {
        this.resources = resources.map(r => new IdeaX.RCResource(r));
   ...
   */
  public async openResource(resource: IdeaX.RCResource) {
   ...
   */
  public actionsOnResource(res: IdeaX.RCResource) {
   ...
   */
  public browseUpdateResource(res: IdeaX.RCResource) {
    if (!this.admin) return;
    // browse a local file
   ...
   * Rename a resource.
   */
  protected renameResource(res: IdeaX.RCResource) {
    this.alertCtrl
      .create({
   ...
   * Delete a resource.
   */
  protected deleteResource(res: IdeaX.RCResource) {
    this.alertCtrl
      .create({
  ...
  public async uploadNewResources(ev: any) {
    ...
    Async.each(
      files,
      (file, doneFile) => this.uploadFile(file, doneFile),
      () => {
        this.loading.hide();
        if (this.uploadErrors.length) this.message.error('IDEA.RESOURCE_CENTER.ONE_OR_MORE_FILE_UPLOAD_FAILED');
        else this.message.success('IDEA.RESOURCE_CENTER.UPLOAD_COMPLETED');
   ...
   */
  protected uploadFile(file: File, cb: any, existingRes?: IdeaX.RCResource) {
    // prepare the resource metadata
    const fullName = file.name.split('.');
    const format = fullName.pop();
    const name = fullName.join('.');
    let resource: IdeaX.RCResource;
    if (existingRes) {
      existingRes.format = format as IdeaX.RCResourceFormats;
      resource = existingRes;
    } else resource = new IdeaX.RCResource({ name, format });
    // check the file format
    if (!IdeaX.loopStringEnumValues(IdeaX.RCResourceFormats).some(x => x === format)) {
    ...
    let req: Promise<IdeaX.RCResource>;
