...
export class IDEAEchoPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    switch (request) {
      ...
      default:
        this.success = false;
        this.content = this._translate._('IDEA_COMMON.ECHO.INVALID_ACTION');
        break;
    }
  }
  ...
  async followRegistrationLink(code: string, user: string): Promise<void> {
    try {
      ...
      this.success = true;
      this.content = this._translate._('IDEA_COMMON.ECHO.ACCOUNT_CONFIRMED');
    } catch (error) {
      this.success = false;
      this.content = this._translate._('IDEA_COMMON.ECHO.REQUEST_FAILED');
    } finally {
      this._loading.hide();
    }
  }
  async followInvitationLink(code: string): Promise<void> {
    try {
      ...
      this.success = true;
      this.content = this._translate._('IDEA_COMMON.ECHO.TEAM_JOINED');
    } catch (error) {
      this.success = false;
      this.content = this._translate._('IDEA_COMMON.ECHO.REQUEST_FAILED');
    } finally {
  ...
  async followEmailChangeConfirmationLink(code: string): Promise<void> {
    try {
      ...
      this.success = true;
      this.content = this._translate._('IDEA_COMMON.ECHO.EMAIL_CHANGED');
    } catch (error) {
      this.success = false;
      this.content = this._translate._('IDEA_COMMON.ECHO.REQUEST_FAILED');
    } finally {
  ...
  endGitHubIntegrationFlow(success: boolean): void {
    ...
    this.content = success
      ? this._translate._('IDEA_COMMON.ECHO.GITHUB_SOURCE_INTEGRATION_SUCCESS')
      : this._translate._('IDEA_COMMON.ECHO.GITHUB_SOURCE_INTEGRATION_ERROR');
  }
  endTrelloIntegrationFlow(token: string): void {
    this.success = Boolean(token);
    this.content = token
      ? this._translate._('IDEA_COMMON.ECHO.TRELLO_SOURCE_INTEGRATION_SUCCESS', { token })
      : this._translate._('IDEA_COMMON.ECHO.TRELLO_SOURCE_INTEGRATION_ERROR');
  }
  async endExternalCalendarsIntegrationFlow(
    ...
    try {
      ...
      this.success = true;
      this.content = this._translate._('IDEA_COMMON.ECHO.EXTERNAL_CALENDARS_SOURCE_INTEGRATION_SUCCESS');
    } catch (error) {
      this.success = false;
      this.content = this._translate._('IDEA_COMMON.ECHO.EXTERNAL_CALENDARS_SOURCE_INTEGRATION_ERROR');
    } finally {
      this._loading.hide();
    }
  }
